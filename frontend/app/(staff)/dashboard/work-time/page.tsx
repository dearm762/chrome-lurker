...
export default function AddUser() {
	...
	const setWorkTime = (e: FormEvent) => {
		...
		fetch(urlCreator('setting/1'), {
			...
			body: JSON.stringify({ to: formData.to, fromm: formData.from }),
		})
	}
			...
			<div className='min-h-screen pt-32 bg-[#F2F2F2]'>
				<div className='w-full max-w-[1280px] mx-auto flex flex-col pt-10'>
					<div className='bg-white rounded-e-lg'>
						<Heading
							text={locales[lang]['work-time'].title}
							additionalStyle='text-center mt-20'
						/>
						<Info text={locales[lang]['work-time'].description} />
						...
						<div className='flex justify-center '>
							...
							>
								{locales[lang]['work-time']['back-Button']}
							</Link>
							<button
								className='bg-black ml-3 rounded-md mt-6 py-3 text-white px-10 mb-10'
								onClick={e => setWorkTime(e)}
							>
								{locales[lang]['work-time']['save-Button']}
							</button>
						</div>
					</div>
				</div>
